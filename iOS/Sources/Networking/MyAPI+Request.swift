...
enum MyAPIError: Error {
    ...
    var isNoNetwork: Bool {
        ...
        case let .restError(error, _, _):
            return NextStapAPI.isNotConnection(error: error) || NextStapAPI.isLostConnection(error: error)
...
extension NextStapAPI {
    ...
    private enum MoyaWrapper {
        ...
        static var provider: MoyaProvider<NextStapAPI.Wrapper> {
            ...
            return MoyaProvider<NextStapAPI.Wrapper>(
...
extension NextStapAPI {
    ...
    func request(
        ...
        let endpoint = NextStapAPI.Wrapper(base: self)
